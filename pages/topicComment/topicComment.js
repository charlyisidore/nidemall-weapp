  ...
  loadCommentList() {
    util.request(api.CommentList, {
      ...
      .then((res) => {
        ...
        if (0 == this.data.showType) {
          const allCommentList = this.data.allCommentList.concat(res.data.list);
          this.setData({
            allCommentList,
            allPage: res.data.page,
            comments: allCommentList,
          });
        } else {
          const picCommentList = this.data.picCommentList.concat(res.data.list);
          this.setData({
            picCommentList,
            picPage: res.data.page,
            comments: picCommentList,
          });
        }
      });
  },
